...
def main():
    ...
    ap.add_argument('--fake', action='store_true', help='Generate a tiny fake submission in-memory')
    args = ap.parse_args()
    if args.fake:
        import pandas as pd
        ...
        validate(df); return
    if not args.path:
        print('Usage: python -m tools.check_submission --path submission.csv', file=sys.stderr); sys.exit(2)
